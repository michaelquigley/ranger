...
`Load` enumerates `roadmap/*.md` flat — skipping directories, non-markdown, and `.capture-` temps — and reads `.ranger/order.yaml`. Error tiers per the spec: a missing or unreadable roadmap directory and an unreadable `order.yaml` are repository-level errors; a single bad item degrades to a flagged card. The snapshot carries each item's raw bytes, guard hash, and parsed document, plus the order document and its version (absence is the `absent` sentinel). `Cards()` classifies everything for `ComputeBoard`, attaching the malformed and filename-mismatch flags.

`Load` also reads `.ranger/filters.yaml` — the saved-filter sidecar, beside the ranking in the same tool-state directory. Its error tier is softer than order.yaml's: an unreadable filters file degrades to an unknown version (`FiltersVersion` empty) rather than failing the board, because a broken lens must not take down the board it views. `SaveFilters` replaces the set whole — a fresh render under the version guard — the one write that is a re-render rather than a patch, because the file is tool-rendered rather than operator-authored. `.ranger/` itself is created by the two writes that put files in it, so a roadmap with neither ranking nor saved filters never grows a directory it has no use for.

## the order migration

`order.yaml` used to live at the roadmap root, beside the items; it moved into `.ranger/` (design change 2026-08-14) once that directory existed and the ranking proved to be a file the operator manipulates through the board rather than by hand. `Load` migrates a legacy one forward in place, once, before it reads. It is a move, never a rewrite — the operator's bytes cross unchanged, comments, spacing, and inert lines all surviving, and git reads the working-tree change as the rename it is. This is the one write ranger performs without being asked for it, and the alternative is worse: a board that silently drops the ranking of every roadmap written before the move. A migration that cannot complete is repository-level, the same tier as an unreadable order.yaml — there is no fallback path that reads the legacy location, so the file has exactly one home at every moment after startup.

The move goes through `FinalizeLink`, the same no-clobber funnel capture and rename use, so a `.ranger/order.yaml` already in place wins by refusal rather than by a hopeful check — the destination is claimed atomically by `link(2)`, never by a check the next syscall can invalidate. A collision leaves the stray legacy file exactly where it sits for the operator to resolve, because past the migration that state can only be hand-made. A collision and a source that vanished under a concurrent loader are both success: the ranking arrived either way. An entry named `order.yaml` that isn't a file is refused by the same primitive and surfaces as a repository-level error rather than being quietly rearranged.

## the gesture discipline

Every gesture follows one shape: load fresh, verify every affected guard token against that fresh read (a mismatch is a typed conflict — reload, don't retry), compute the minimal bytes that express the gesture, then write — item first, order second, any partial failure reported plainly. Opportunistic pruning rides along only when `order.yaml` is already being written. Order writes go through one funnel that creates `.ranger/` first, so a first-ever ranking is never refused for want of a directory rather than for want of a guard match.
