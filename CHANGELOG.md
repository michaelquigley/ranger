...
FEATURE: "not" filters — alt-clicking a tag chip, subsystem chip, or milestone badge filters it *out*: cards carrying the excluded value drop from the view, and the header pill reads `not x`. Plain click still includes; a value clicked into the opposite polarity flips rather than stacking, the milestone slot stays single-select across both polarities, and excluding a milestone keeps unmilestoned cards. Client-side presentation only, composing with every existing filter and the search box.

CHANGE: the item body has a documented shape — everything above the first `##` heading is the prompt, and supporting material that isn't the prompt (`## why` for justification, `## background` for a longer description) lives in named sections below it. The modal gives the prompt primacy accordingly: the prompt sized a step up, supporting sections a step down behind a rule and under quieter headings. Presentation only — sections are conventional and never validated, nothing parses prose to find them, no frontmatter field was added, and every existing item renders exactly as before.

FEATURE: the board reloads itself — a quiet two-second poll runs the same fresh board-and-index reads a manual refresh runs, so changes from any hand (editor, CLI, agent, another window) appear on their own, dirty markers and project availability included. The poll is visibility-gated (a hidden tab is silent; returning to one triggers an immediate tick), never applies mid-drag, and preserves object identity on unchanged reads so a quiet tick repaints nothing. An open item modal rides the same clock — the fresh board's hash for the item is the change signal, so an edit from any other hand re-renders the open item too, while raw edit, retitle, and delete-confirm are never stomped; an item deleted elsewhere surfaces its 404 plainly. Manual refresh still works and is never required.

FEATURE: dirty git highlights — the board shows what's uncommitted. A card whose file is dirty (modified, staged, or untracked alike) carries a quiet red left-edge accent, the board reports a roadmap-wide verdict (order.yaml and assets included), and the project selector suffixes dirty boards with the git vernacular's `*`, so the open list answers "is everything committed?" across every board at once. The verdict rides three optional wire fields (card, board, and project index), judged by a read-only `git status` scoped to the roadmap directory, fresh per request; where git can't answer — no binary, a root outside any repository — the fields are absent and the UI shows nothing, never a pretended cleanliness. This narrows the never-touches-git rule to never-*writes*-git (a recorded design change): the tool reads status and nothing more — no git library, no staging, no commits, ever.
